...
class BaseBinaryClassifierLightning(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        precision = self.precision(probs, labels)
        recall = self.recall(probs, labels)
        ...
        self.log(
            "train_auc",
            np.round(auc.detach().cpu().numpy(), 3),
            prog_bar=True,
            on_step=False,
            on_epoch=True,
        )
        self.log(
            "train_precision",
            np.round(precision.detach().cpu().numpy(), 3),
            prog_bar=True,
            on_step=False,
            on_epoch=True,
        )
        self.log(
            "train_recall",
            np.round(recall.detach().cpu().numpy(), 3),
            prog_bar=True,
            on_step=False,
            on_epoch=True,
        )
        self.log(
            "train_f1",
            np.round(f1.detach().cpu().numpy(), 3),
            prog_bar=True,
            on_step=False,
            on_epoch=True,
        )
